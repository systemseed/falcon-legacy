...
namespace Drupal\cw_payment_event\Controller;
 ...
 *
 * @package Drupal\cw_payment_event\Controller
 */
class StripeController extends ControllerBase {
  ...
  function createPaymentEventEntities() {
    try {
      ...
      $stripe_username = \Drupal::config('cw_payment_event.' . $paymentGateway)->get('developer_username');
      ...
      watchdog_exception('cw_payment_event', $e);
      return new EmptyResponse(500);
    }
  }
  ...
  private function loginToStripe($cookieJar, $csrfToken, $paymentGateway) {
    ...
    $config = \Drupal::config('cw_payment_event.' . $paymentGateway);
  ...
  function createEntities($paymentGateway, $activities) {
    ...
    if (count($created_events)) {
      ...
      \Drupal::logger('cw_payment_event')
  ...
  function sendEmail($paymentGateway, $activity) {
    $to = \Drupal::config('cw_payment_event.' . $paymentGateway)->get('account_updates_notification_emails');
    if (empty($to)) {
      return;
    }
    ...
    $message =  \Drupal::service('plugin.manager.mail')->mail('cw_payment_event', 'stripe_account_changed', $to, $langcode, $params);

    // Add message to the log if email has been sent.
    if (!empty($message['result'])) {
      \Drupal::logger('cw_payment_event')->notice('Stripe notification has been successfully sent.');
    }
    else {
      \Drupal::logger('cw_payment_event')->notice('There was a problem sending Stipe notification message.');
    }
  }
}
